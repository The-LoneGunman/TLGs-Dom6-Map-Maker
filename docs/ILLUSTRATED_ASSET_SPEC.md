...
Reviewed September 22, 2026 against merged source `464ff03`. The package version remains 0.1.5. Its published `581b2b8` build includes the four material textures; connected regions, procedural realm painters and the optional illustrated game exporter described below are now on `main` but not deployed or released. See [Versions and documentation](../README.md#versions-and-documentation).
...
Cloud/Air planes use `src/skyArt.ts` for cloud banks, floating-island edges/shadows, and owner-clipped terrain motifs. `src/realmArt.ts` supplies deterministic textured rock relief, fungal/crystal details, Underworld tomb stones and mist, infernal ember fissures, abyssal obsidian, dream vegetation, and variant-led elemental accents. Both renderers sample canonical ownership at display density; PNGs use native dimensions. Effective terrain flags remain authoritative, so actual Sea remains blue with aquatic-colored details even in an infernal realm, and Cave Wall remains sealed rock. The new realm themes add no winter snow; Cloud/Air retain their existing eligible dry-island policy. These shared procedural renderers require no external artwork or new dependencies and also supply the illustrated exporter described below.

Artwork appears automatically in the editor and PNG previews. The optional **Illustrated realms** export now includes these procedural images for supported realms, alongside canonical `#pb` province areas and remapped gameplay references. Surface and Custom planes remain native. The default **Native scenery** export still supplies D6M geography for every plane. See the [illustrated export contract and game checks](ILLUSTRATED_EXPORT_2026-09-22.md); the bundled grayscale material/backdrop files themselves remain preview assets.
...
Condition previews resolve a separate, non-mutating terrain mask. Submerged preserves cave identity while adding water; transformed cover removes incompatible vegetation, and cave walls remain unchanged. Winter cover is an illustrative policy, not a temperature simulation: water and caves are excluded; Cloud/Air retain eligible dry-island snow; the newly illustrated Underworld, Infernal, Abyss, Dream, and Elemental themes add none. Neither editor nor PNG appearance certifies the game's exact seasonal rendering.
...
A shipped pack should remain automatic: users should not need to download, resize, place, or repair its artwork. The current illustrated exporter supplies its own generated sheets automatically; arbitrary future asset packs would still need explicit format, clipping, terrain-transition and licensing support. Native D6M mode does not embed preview artwork.
