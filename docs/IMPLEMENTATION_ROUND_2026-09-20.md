...
Implementation commit: [`950481f`](https://github.com/The-LoneGunman/TLGs-Dom6-Map-Maker/commit/950481fbfbf4434172e84a9ceb68f9861ee5f4e7). Integrated into `main` at the user's request on September 20, with the native-game check deferred. Source integration does not deploy the hosted app or publish a Windows release.
...
[GitHub CI for implementation commit `950481f`](https://github.com/The-LoneGunman/TLGs-Dom6-Map-Maker/actions/runs/35548820433) passed before the source merge: the automated release suite, type checking, lint, dependency audit, license verification, local-server smoke test, and Windows launcher checks all succeeded.
...
The installed game opened a black window before a usable menu. Both its normal and documented simple rendering paths showed this behavior; it also persisted with an **empty test-map directory and no Atlas map selected**. Logs identify OpenGL initialization but do not establish a successful game load. The user plans to inspect the game in the morning and explicitly authorized committing, pushing, and merging current progress in the meantime. This is **not a passed in-engine test**. No current-source seasonal, recruitment, post-capture defence, or guardian-combat acceptance is claimed.
